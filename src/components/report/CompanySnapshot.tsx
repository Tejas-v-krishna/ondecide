  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-navy-800 p-6">
      <div className="flex flex-col sm:flex-row sm:items-start gap-4">
        {/* Logo */}
        {data.logo ? (
          <div className="w-14 h-14 rounded-xl border border-slate-700 bg-white overflow-hidden flex-shrink-0">
            <Image src={data.logo} alt={data.name} width={56} height={56} className="w-full h-full object-contain p-1" />
          </div>
        ) : (
          <div className="w-14 h-14 rounded-xl border border-slate-700 bg-slate-800 flex items-center justify-center flex-shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1">
            ...
            <span className="font-mono text-sm text-slate-500 bg-slate-800 px-2 py-0.5 rounded">{data.ticker}</span>
      ...
      {data.marketCap > 0 && (
        <div className="mt-3 pt-3 border-t border-slate-800">
          ...
          <span className="text-sm font-medium text-slate-300">{formatMarketCap(data.marketCap, data.currency)}</span>
        </div>
      )}

      {/* Description */}
      <p className="mt-4 text-slate-300 leading-relaxed text-sm sm:text-base">{data.description}</p>
